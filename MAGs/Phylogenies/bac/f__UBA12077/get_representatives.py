...
for genome in representatives:
    ...
    outputting.write('cp ${GTDBTK_DATA_PATH}fastani/database/' + genome.replace('GB_','').replace('RS_','').split('_')[0] + '/' + genome.replace('GB_','').replace('RS_','').split('_')[1][0:3] + '/' + genome.replace('GB_','').replace('RS_','').split('_')[1][3:6] + '/' + genome.replace('GB_','').replace('RS_','').split('_')[1][6:9] + '/' + genome.replace('GB_','').replace('RS_', '') + '_genomic.fna.gz ' + dir_path + '/Genome_analysis/' + genome  + '--' + genome + '_genomic.fna.gz')
    outputting.close()
    process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
    output, error = process.communicate()


for i in glob.glob(dir_path + '/Genome_analysis/*.gz'):
...
for cfile in glob.glob(dir_path +'/Genome_analysis/*.fna'):
    bashCommand = 'mkdir -p ' + dir_path + '/Genome_analysis/'+cfile.split('/')[-1:][0].replace('.fna','')
    process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
    output, error = process.communicate()
    bashCommand = 'prodigal -i ' + cfile + ' -c -m -a ' + dir_path + '/Genome_analysis/'+cfile.split('/')[-1:][0     ].replace('.fna','') + '/' + cfile.split('/')[-1:][0].replace('.fna','') + '.faa'                    #########
    process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
    output, error = process.communicate()


genome_file = "mags/bin146-contigs.fa"
